...
//TreeMaps extends AbstractMap and implementt NaviguableMap interface
public class Case_TreeMap {
	
	public static void main(String...args){
		//Instanciate a TreeMap to hold people names and They ages 
		...
		//getting first an las keys
		System.out.println("The first key in the Map is:"+ages.firstKey());
		System.out.println("The last Key in the Map is:"+ages.lastKey());
		
		//Retreiving portion of the Map
		// retrieving a heaMap
		SortedMap<String,Integer> sorted_head_map = ages.headMap("Edmon");//specify the End key parameter.It excludes argument's entry
		                         Set<String> headKeySet = sorted_head_map.keySet();//return a set
		                         Collection <Integer> headValueSet = sorted_head_map.values();//returna raw collection
		                         System.out.println("Key up to Edmon: "+headKeySet);
		                         System.out.println("values up to Edmon: "+headValueSet);
		 
		//Retrieving a tailMap
		SortedMap<String,Integer> sorted_tail_map = ages.tailMap("Fatim");//specify the Start key parameter include argument's entry
		                         Set<String> tailKeySet = sorted_tail_map.keySet();//return a set
                                 Collection <Integer> tailValueSet = sorted_tail_map.values();//returna raw collection
                                 System.out.println("Key from Fatim: "+tailKeySet);
                                 System.out.println("values form Fatim: "+tailValueSet);
		
        //Retrieving a subMap                         
		SortedMap<String,Integer> sorted_subMap = ages.subMap("Brahima","Jake");//specify the Start and End(start included,end excluded)
		                         Set<String> subKeySet = sorted_subMap.keySet();//return a set
                                 Collection <Integer> subValueSet = sorted_subMap.values();//returna raw collection
                                 System.out.println("Key between Barhima and Jake: "+subKeySet);
                                 System.out.println("values form Fatim: "+subValueSet);
                                 
        //retrieving Entries, based on a meduim
          Map.Entry<String, Integer> ceil_entry = ages.ceilingEntry("Emile");
        // the key Emile does not exist, this directly adresses the alphabetical key >= Emile
       ...
       // we can directly get the key of the celling or the floor from a Map witout focussing on entries
       ...
       //Get a NaviguableMap from The current Map and print it
          NavigableMap<String,Integer> age_navHeadMap = ages.headMap("Edmon",true);// true iclude data for Edmon
         ...
         //the NavigableMap is an interface, then backed by the TreeMap,Another modification one one affaects the other.
         //In the Following Brahima will removed from both The Navigable submap and the intial TreeMap
          ...
          //We can get the Keysets or a Naviguable Map in descending order.
          NavigableMap<String,Integer> age_desc_navHeadMap = ages.descendingMap();// true iclude data for Edmon
